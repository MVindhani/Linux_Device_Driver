...
int cal_open(struct inode *inode, struct file *filp)
{
	...
	down(&my_semaphore); /* Lock the semaphore. Every device first comes to open.*/
	switch(minor)
	{
		case 0:
...
int cal_release(struct inode *indoe, struct file *filp)
{
	up(&my_semaphore); /* Unlock the semaphores. Every device exits via release function. */
...
static int calculator_entry(void)
{
	...
	else
	{
		...
		sema_init(&my_semaphore,1); /* Init semaphore with value 1.*/
